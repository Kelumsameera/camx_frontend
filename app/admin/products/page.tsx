...
import { Package, Search, Plus, Trash2, Edit, RefreshCw, Layers, CircleDollarSign } from 'lucide-react';
...
export default function AdminProductsPage() {
  ...
  const handleDeleteProduct = async (productId: string) => {
    if (!window.confirm('Are you absolutely sure you want to delete this product asset from the database?')) {
      return;
    }
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const brandMatch = product.brand?.toLowerCase().includes(search.toLowerCase()) || false;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-sm text-neutral-500 dark:text-gray-400 mt-2">
            Manage your store repository, tracking assets, update pricing stocks, and remove hardware models.
          </p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 animate-pulse">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-72 bg-neutral-100 dark:bg-card border border-neutral-200 dark:border-border rounded-3xl" />
          ))}
        </div>
        ...
        <div className="text-center py-20 bg-white dark:bg-card border border-border rounded-3xl p-8">
          ...
          <h2 className="text-xl font-bold text-neutral-900 dark:text-white">No products found</h2>
          <p className="text-sm text-neutral-500 dark:text-gray-400 mt-1">There are no matching hardware items registered inside the local repository layout filters.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="relative aspect-4/3 bg-neutral-50 dark:bg-neutral-950 p-4 flex items-center justify-center border-b border-neutral-100 dark:border-border/40 overflow-hidden">
                ...
                {product.stock !== undefined && (
                  <span className={`absolute bottom-3 left-3 px-2 py-0.5 rounded-md text-[10px] font-black border ${product.stock > 0 ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-neutral-400">
                    <span className="flex items-center gap-1"><Layers size={10} /> {product.category}</span>
                  ...
                  {product.model && (
                    <p className="text-[11px] font-medium text-neutral-400 dark:text-gray-500 truncate">Mod: {product.model}</p>
                  )}
                </div>
                ...
                <div className="pt-3 border-t border-neutral-100 dark:border-border/40 flex items-center justify-between mt-1">
                  <div className="flex flex-col">
                    <span className="text-xs font-semibold text-neutral-400 flex items-center gap-0.5"><CircleDollarSign size={10} /> Net Price</span>
                    <span className="text-base font-black text-secondary">LKR {product.price.toLocaleString()}</span>
                  </div>
